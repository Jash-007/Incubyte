...
public class IncubyteProblem {
    public static void main(String[] args) {
        ...
        int initial_x = scanner.nextInt();

        System.out.print("Enter the initial y coordinate: ");
        int initial_y = scanner.nextInt();

        System.out.print("Enter the initial z coordinate: ");
        int initial_z = scanner.nextInt();

        System.out.print("Enter the initial direction (N, S, E, W, U, D): ");
        char initial_dir = scanner.next().charAt(0);

        List<Character> commands = new ArrayList<>();
        System.out.print("Enter the commands (f, b, l, r, u, d): ");
        String commandsString = scanner.next();
        for (char command : commandsString.toCharArray()) {
            commands.add(command);
        }

        Craft chandrayaan_3 = new Craft(initial_x, initial_y, initial_z, initial_dir);
        CraftManage.Commands(chandrayaan_3, commands);

        System.out.println("Final Position: (" + chandrayaan_3.x + ", "
                + chandrayaan_3.y + ", " + chandrayaan_3.z + ")");
        System.out.println("Final Dir: " + chandrayaan_3.dir);
